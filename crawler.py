...
def crawl(api, hashtag, config, mode='initial'):
	if mode == 'initial':
		if os.path.exists(config['profile_path'] + os.path.sep):
			if os.path.exists(config['profile_path'] + os.path.sep + hashtag):
				if len(os.listdir(config['profile_path'] + os.path.sep + hashtag)) > 0:
					print('There exists previous files. Now starting surface crawling.')
					print('DIR: ' + config['profile_path'] + os.path.sep + hashtag)
					mode = 'surface'
...
def save_partial(api, hashtag, config, feed, prev_time=None):
	...
	try:
		...
		if not os.path.exists(config['profile_path'] + os.path.sep + hashtag):
			os.makedirs(config['profile_path'] + os.path.sep + hashtag)
	except Exception as e:
	...
	try:
		# with open(config['profile_path'] + os.path.sep + str(hashtag) + os.path.sep + file_name + '.json', 'w') as file:
...
def latest_time(hashtag, config):
	...
	try:
		...
		print('exception in profile path')
		raise e
	file_list = os.listdir(config['profile_path'] + os.path.sep + hashtag)
	file_list.sort()
